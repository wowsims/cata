...
spell_url_from_item_id() {
	safe_curl "https://www.wowhead.com/mop-classic/item=$1" |
		grep -o '\/mop-classic\/spell=[0-9]\+' |
		head -n 1
}
...
spell_effect_from_url() {
	safe_curl "https://www.wowhead.com$1" |
		grep -oP '<th>Effect.*?</th>.*<a href="\/mop-classic\/spell=[0-9]+/.*?">\K.*?(?=</a>)|<th>Effect.*?</th>.*<span class="q2">\K.*(?=</span>)'
}
...
grep '^\s*{' |
	while read -r line; do
		...
		if echo "$line" | grep 'IsSpellID: true' >/dev/null; then
			effect=$(spell_effect_from_url "/mop-classic/spell=$id")
		else
