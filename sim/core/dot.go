...
func calculateHastedTickCount(baseDuration time.Duration, tickPeriod time.Duration) int32 {
...
func (dot *Dot) HastedTickCount() int32 {
	return calculateHastedTickCount(dot.BaseDuration(), dot.tickPeriod)
}
...
func (dot *Dot) ExpectedTickCount() int32 {
	...
	if dot.affectedByCastSpeed && !dot.hasteReducesDuration {
		...
		tickCount = calculateHastedTickCount(dot.BaseDuration(), tickPeriod)
	}
	return tickCount
}
