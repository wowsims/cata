...
func UpdateVengeance(sim *Simulation, character *Character, tracker *VengeanceTracker, aura *Aura) {
	// Save the current AP bonus so the aura can reapply things correctly
...
func ApplyVengeanceEffect(character *Character, tracker *VengeanceTracker, spellID int32) {
	...
	vengAura := MakePermanent(character.RegisterAura(Aura{
		...
		// OnRefresh rather than OnExpire/OnGain to avoid extra event churn and more closely match game behavior
		// Vengeance is a persistent buff whose effect updates with each tick rather than having it reapplied
		OnRefresh: func(aura *Aura, sim *Simulation) {
