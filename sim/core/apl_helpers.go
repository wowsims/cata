...
func (rot *APLRotation) GetAPLItemProcAuras(statTypesToMatch []stats.Stat, minIcd time.Duration, warnIfNoneFound bool, isPrepull bool, uuid *proto.UUID) []*StatBuffAura {
	unit := rot.unit
	character := unit.Env.Raid.GetPlayerFromUnit(unit).GetCharacter()
	matchingAuras := character.GetMatchingItemProcAuras(statTypesToMatch, minIcd, isPrepull)
