...
var ItemSetNecroticBoneplateArmor = core.NewItemSet(core.ItemSet{
	...
	Bonuses: map[int32]core.ApplySetBonus{
		2: func(agent core.Agent, setBonusAura *core.Aura) {
			// When an attack drops your health below 35%, one of your Blood Runes will immediately activate and convert into a Death Rune for the next 20 sec.
			// This effect cannot occur more than once every 45 sec.
			dk := agent.(DeathKnightAgent).GetDeathKnight()

			actionID := core.ActionID{SpellID: 105582}
			bloodMetrics := dk.NewBloodRuneMetrics(actionID)
			deathMetrics := dk.NewDeathRuneMetrics(actionID)
			kissOfDeathAura := dk.RegisterAura(core.Aura{
				Label:    "Kiss of Death" + dk.Label,
				ActionID: core.ActionID{SpellID: 105582},
			...
			setBonusAura.AttachProcTrigger(core.ProcTrigger{
				Name:     "Kiss of Death Trigger" + dk.Label,
				Callback: core.CallbackOnSpellHitTaken,
				Harmful:  true,
				ICD:      time.Second * 45,

				ExtraCondition: func(sim *core.Simulation, spell *core.Spell, result *core.SpellResult) bool {
					return dk.CurrentHealthPercent() < 35
				},

				Handler: func(sim *core.Simulation, spell *core.Spell, result *core.SpellResult) {
					if dk.CurrentHealthPercent() < 35 {
						kissOfDeathAura.Activate(sim)
					}
				},
			})
		},
