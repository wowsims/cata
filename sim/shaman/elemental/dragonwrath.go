...
func init() {
	...
	cata.CreateDTRClassConfig(proto.Spec_SpecElementalShaman, 0.108).
		...
		AddSpell(77478, cata.NewDragonwrathSpellConfig().IsAoESpell().TreatTickAsCast().WithSpellHandler(customEarthquakeHandler)) // Earthquake
}
...
func customEarthquakeHandler(sim *core.Simulation, spell *core.Spell, result *core.SpellResult) {
	...
	if copySpell == nil {
		copyConfig := cata.GetDRTSpellConfig(spell)
		copyConfig.Cast = core.CastConfig{}
		copyConfig.Flags |= core.SpellFlagIgnoreResists // Doesnt get double damage reduced by armor
		copySpell = spell.Unit.RegisterSpell(copyConfig)
	}
