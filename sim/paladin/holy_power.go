...
func (pb *HolyPowerBar) GainHolyPower(sim *core.Simulation, amountToAdd int32, metrics *core.ResourceMetrics) {
	...
	if sim.Log != nil {
		sim.Log("Gained %d holy power from %s (%d --> %d) of %0.0f total.", newHolyPower, metrics.ActionID, pb.holyPower, newHolyPower, 3.0)
	}
...
func (pb *HolyPowerBar) SpendHolyPower(sim *core.Simulation, metrics *core.ResourceMetrics) {
	...
	if sim.Log != nil {
		sim.Log("Spent %d holy power from %s (%d --> %d) of %0.0f total.", pb.holyPower, metrics.ActionID, pb.holyPower, 0, 3.0)
	}
