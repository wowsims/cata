	...
	*/
	...
	filterItemData<T>(itemData: Array<T>, getItemFunc: (val: T) => Item, slot: ItemSlot): Array<T> {
		...
		if (filters.minIlvl != 0) {
			itemData = filterItems(itemData, item => item.scalingOptions[ItemLevelState.Base].ilvl >= filters.minIlvl);
		}
		if (filters.maxIlvl != 0) {
			itemData = filterItems(itemData, item => item.scalingOptions[ItemLevelState.Base].ilvl <= filters.maxIlvl);
		}
