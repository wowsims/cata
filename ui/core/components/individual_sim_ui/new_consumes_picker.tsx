...
import { ConsumableSubType, ConsumableType } from '../../proto/common';
...
export class ConsumesPicker extends Component {
	...
	private async buildElixirsPicker(): Promise<void> {
		...
		const battleElixirs = this.db
			.getConsumablesByTypeAndStats(ConsumableType.ConsumableTypeElixir, this.simUI.individualConfig.epStats)
			.filter(con => con.subType == ConsumableSubType.ConsumableSubTypeBattleElixir);
		const battleElixirOptions = ConsumablesInputs.makeConsumableInput(battleElixirs, { consumesFieldName: 'battleElixirId' }, '');

		const guardianElixirs = this.db
			.getConsumablesByType(ConsumableType.ConsumableTypeElixir)
			.filter(con => con.subType == ConsumableSubType.ConsumableSubTypeGuardianElixir);
