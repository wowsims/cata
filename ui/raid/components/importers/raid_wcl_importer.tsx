...
import { Party, PlayerStats, Raid } from '../../../core/proto/api';
import {
	Class,
...
export class RaidWCLImporter extends RaidImporter {
	...
	private getRaidProto(wclPlayers: WCLSimPlayer[]): Raid {
		const raid = Raid.create({
			parties: [...new Array(MAX_NUM_PARTIES).keys()].map(_party =>
				Party.create({
					players: [...new Array(5).keys()].map(() => PlayerStats.create()),
				}),
			),
		});
